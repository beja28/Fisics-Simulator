...
class Viewer extends SimulationViewer {
	...
	protected void paintComponent(Graphics g) {
		...
		gr.drawLine(_centerX - crossSize, _centerY, _centerX + crossSize, _centerY); //linea horizonal
		gr.drawLine(_centerX, _centerY - crossSize, _centerX, _centerY + crossSize);// linea vertical
		// draw bodies
		drawBodies(gr);
	...
	private void showHelp(Graphics2D g) {
		...
		String text = "h: toggle help, v: toggle vectors, +: zoom-in, -: zoom-out, =: fit\n" +
                "l: move right, j: move left, i: move up, m: move down: k: reset\n" +
                "g: show next group\n" +
                "Scaling ratio: " + _scale + "\n" +
                "Selected Group: " + grupo;
        // Establecer la fuente de texto y el color
        ...
        // Mostrar el texto en la posición calculada
	...
	private void drawBodies(Graphics2D g) {
		...
		for(Body b: _bodies) {
			if(isVisible(b)) {
				int PosX, PosY;
				PosX = (int)(_centerX + b.getPosition().getX()/_scale);
				PosY = (int)(_centerY + b.getPosition().getY()/_scale);
				g.setColor(_gColor.get(b.getgId()));
				g.fillOval(PosX-(13/2),PosY-(13/2), 13, 13);
				g.setColor(Color.black);
				g.drawString(b.getId(), PosX-6 , PosY-9);
				if(_showVectors == true) {
					Vector2D v = b.getVelocity().direction().scale(25);
					Vector2D f = b.getForce().direction().scale(25);
					int vX = (int)(PosX + v.getX());
					int vY = (int)(PosY + v.getY());
					int fX = (int)(PosX + f.getX());
					int fY = (int)(PosY + f.getY());
					//Dibujar el vector de Fuerza
					drawLineWithArrow(g, PosX, PosY ,
							fX, fY, 3, 3, Color.RED, Color.RED);
					//Dibujar el vector de Velocidad
					drawLineWithArrow(g, PosX, PosY , 
							vX, vY, 3, 3, Color.GREEN, Color.GREEN);
				}
	...
	private boolean isVisible(Body b) {
		...
		if(_selectedGroup == null || _selectedGroup.equalsIgnoreCase(b.getgId())) {
			visible = true;
		}
	...
	public void addGroup(BodiesGroup g) {
        ...
        while(!encontrao && i < _groups.size()) {
            if(_groups.get(i).getId().equalsIgnoreCase(g.getId())) {
                encontrao = true;
            }
            else {
                i++;
	...
	public void addBody(Body b) {
		int i = 0;
        //boolean encontrao = false;
        /*while(!encontrao && i < _bodies.size()) {
        	if(_bodies.get(i).getId().equalsIgnoreCase(b.getId())) {
                encontrao = true;
            }
        	else
        	{
