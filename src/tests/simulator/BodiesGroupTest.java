...
class BodiesGroupTest {
	...
	void basic_behaviour() {
		...
		assertEquals(new Vector2D(-200.3340550263439, -612.7988936867671), b4.getForce(),
				"MovingBody.getForce returned a wrong value");
		assertTrue("MovingBody.getPosition returned a wrong value after calling (group) advance",
				new Vector2D(4.895824312170701, 2.540013828915411).distanceTo(b4.getPosition()) <= 0.1);
		assertTrue("MovingBody.getVelocity returned a wrong value after calling (group) advance",
				new Vector2D(-1.6083513756585979, -14.119972342169179).distanceTo(b4.getVelocity()) <= 0.1);
		...
		assertEquals(new Vector2D(358.2358166275974, -159.15746820416064), b4.getForce(),
				"MovingBody.getForce returned a wrong value");
		assertTrue("MovingBody.getPosition returned a wrong value after calling (group) advance", 
				new Vector2D(-261.49438276099085, 122.92332373265208).distanceTo(b4.getPosition()) <= 0.1);
	...
	@Test
	void errors_handling() throws Exception {
		// id cannot be empty
